...
jq -L "$benchmark_jq_dir" -Rsc \
  ...
  def collated_summary($rows):
    ...
    ($rows | map(.block_limits.bytes? // 0) | add // 0) as $estimated_bytes |
    {
      ...
      native_run_representation:native_run_representation_summary($rows; false),
  ...
  def validated_summary($rows):
    {
      ...
      native_run_representation:native_run_representation_summary([$rows[] | select(.valid? == true)]; true),
      total_time_s:($rows | map(.total_time?) | distribution),
      work_time_s:($rows | map(.work_time?) | distribution),
      ...
      stages_real_s:{
        ...
        native_batch_decode:stage_distribution($rows; "native_batch_decode"),
...
docker inspect genesis "$container_name" session-stats |
  jq '[.[] | {
    ...
    benchmark_environment: [.Config.Env[] | select(test(
      "^(GENESIS_VERBOSITY|GENESIS_HEARTBEAT_SECONDS|ACTUAL_MIN_SPLIT|MIN_SPLIT|MAX_SPLIT|TON_SIMPLEX_[^=]+|TON_NATIVE_[^=]+|TON_KEYRING_PREPARED_SIGNING|TON_OVERLAY_LOCAL_SIGNATURE_REUSE|TD_ACTOR_PROFILE_CPU|NATIVE_(TRANSFER_RUNS|PAYMENT_LANES|PAYMENT_LANE|LOAD_|SPAM_)[^=]*|SIMPLEX_[^=]+|BLOCK_(SIZE|GAS|LIMIT)[^=]*)="
    ))]
  }]' >"$runtime_file"
