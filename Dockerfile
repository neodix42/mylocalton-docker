...
COPY --chmod=744 docker/scripts/native-payment-lanes-config.sh /scripts
COPY --chmod=744 docker/scripts/native-payment-lane-wallets.sh /scripts
...
RUN cp /usr/lib/fift/* /usr/share/ton/smartcont/
